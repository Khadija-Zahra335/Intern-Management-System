...
/**
 * Late-checkin note rule: checking in at or after 10:30 AM PKT requires a
 * short reason (min 5 words), mirroring the late-checkout rule below.
 * Cutoff, minimum length, and the exact message live here as the single
 * source of truth so the API route (enforcement) and the intern attendance
 * page (matching the rejection to show the note form) stay in sync without
 * duplicating the wording.
 */
export const LATE_CHECKIN_HOUR_PKT = 10;
export const LATE_CHECKIN_MINUTE_PKT = 30;
export const LATE_CHECKIN_MIN_WORDS = 5;
export const LATE_CHECKIN_NOTE_MESSAGE =
  "We noticed you're checking in a little late today. No worries — just give a reason for the record.";

/** Trimmed whitespace-separated word count of a possibly-empty note. */
export function noteWordCount(note?: string | null): number {
  return note?.trim().split(/\s+/).filter(Boolean).length ?? 0;
}
